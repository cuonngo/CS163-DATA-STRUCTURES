...
{
	if(strcmp(matching_location, location) == 0)	//check to see if it matched
	{
...
int read(const char prompt[])	//print out the options for the user to choose
{
	...
	cout << prompt;
	cout << "1. Display All\n"
		<< "2. Remove by Location\n"
		<< "3. Retrieve by Location\n"
		<< "4. Display Retrieved Location\n"
		<< "5. Display by Location\n"
		<< "6. Display by Activity\n"
		<< "7. Quit\n"
		<< "Number: ";
...
int journal_entry::get_activity(char * & act)	//this function will pass by reference of the activity
{
	act = activity;
	return 1;
}
